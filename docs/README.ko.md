...
| `x` | 패널 닫기 |
| `z` | 줌 토글 (전체 화면) |
| `R` | 리사이즈 모드 (방향키/hjkl, q 종료) |
| `q` | 패널 번호 표시 + 1-9로 이동 (0은 10번째) |
| `{ }` | 이전/다음 패널과 위치 교환 |
...
- **타이틀 바** — `[━] [┃] [×]` 분할/닫기 버튼 + 실행 중인 명령어 표시
- **줌 모드** — `Ctrl+B z` 또는 더블클릭으로 패널 전체화면
- **키보드 리사이즈** — `Ctrl+B R` → 방향키/hjkl로 크기 조절
- **패널 위치 교환** — `Ctrl+B {` / `}` 로 패널 순서 변경
- **빠른 이동** — `Ctrl+B q` → 번호 표시, 1-9(0은 10번째)로 점프
- **tmux 프리픽스 키** — `Ctrl+B` 후 tmux 키 사용 가능
- **스크롤 모드** — `Ctrl+B [` → vim 키로 히스토리 탐색
- **설정 패널** — `Ctrl+G` → 어두운 모달, vim 키 네비게이션
- **설정 파일** — `~/.config/ezpn/config.toml` 지원
...
| 크기 조절 | `:resize-pane` | 리사이즈 모드 | 드래그 / `Ctrl+B R` |
| Detach | 가능 | 가능 | 불가 |
